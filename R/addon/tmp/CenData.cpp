...
bool CenData::readData(NumericMatrix SurvD)
{
  double max_value = -1.0;
  if(SurvD.nrow()<1||SurvD.ncol()%2==1||SurvD.ncol()<1) return false;
  mDimension = SurvD.ncol()/2;
  for(int ii=0; ii<SurvD.nrow(); ii++){
    for(int i=0; i<(SurvD.ncol()/2); i++){
      cendata_t cendata;
      cendata.x = SurvD(ii,2*i);
      cendata.e = SurvD(ii, 2*i+1);
